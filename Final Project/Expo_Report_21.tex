...
\usepackage{listings}
\lstset{language=[GNU]C++, showstringspaces=false,
...
\begin{figure}[H]
\centering
\includegraphics[width=170mm]{Gantt.eps}
\caption{Gantt Chart}
\end{figure}
...
\begin{figure}[p!]
\subsection{Query Position from Odometry}
\lstinputlisting[language=Python]{odom_query.py}
\centering
\caption{The Python function in navigation which looks up the current x, y, and yaw of the rover via the transforms.}
\end{figure}

\begin{figure}[p!]
\subsection{State Transition}
\lstinputlisting[language=Python]{toggle.py}
\centering
\caption{The Python functions that use a ROS parameter to transition state between the two capstone team's code.}
\end{figure}

\begin{figure}[p!]
\subsection{Setting Up a Movement Goal}
\lstinputlisting[language=C++]{goal.cpp}
\centering
\caption{The C++ function that sets up a movement goal to execute from coordinates and desired orientation. Goals may be either global or local depending on if they come from the overall navigation plan or the home beacon. }
\end{figure}

\begin{figure}[p!]
\subsection{Executing a Movement Goal}
\lstinputlisting[language=C++]{exicute.cpp}
\centering
\caption{The C++ function that executes a movement goal.}
\end{figure}

\begin{figure}[p!]
\subsection{Home Base Service}
\lstinputlisting[language=C++]{home.cpp}
\centering
\caption{The C++ functions that run the home base locator service. These are unique in that the service uses a \texttt{spin()} call to ensure it runs continuously and the service callback contains a \texttt{spinOnce()}, since the OpenCV processing done requires the image to be grabbed through a processing node. The service callback effectively wraps a subscriber node and limits the subscription to a single frame.}
\end{figure}
